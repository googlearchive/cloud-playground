...
describe('cloud playground app', function() {
  ...
  describe('login behavior', function() {
    ...
    it('should only provide login button when the user is not logged in', function() {
      logout();
      expect(element('button:contains("login")').count()).toEqual(1);
      expect(element('button:contains("login")').css('display')).toEqual('inline-block');
    ...
    it('should only provide logout button when the user is logged in', function() {
      login(true);
      expect(element('button:contains("logout")').count()).toEqual(1);
      expect(element('button:contains("logout")').css('display')).toEqual('inline-block');
    ...
    xit('should only provide admin links when user is logged in as an admin', function() {
      ...
      login(true);
      expect(element('[ng-click="big_red_button()"]').height()).toBeGreaterThan(0);
      expect(element('[ng-click="datastore_admin()"]').height()).toBeGreaterThan(0);
      expect(element('[ng-click="memcache_admin()"]').height()).toBeGreaterThan(0);
    });
